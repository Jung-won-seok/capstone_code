...
#include <stdlib.h>
#include <string.h>
#include <sys/stat.h> //파일의 크기, 권한, 생성일시 등 정보를 얻는 함수들
...
int main(int argc, char *argv[])
{
    ...
    while (1)
    {
        ...
        TCP_Connect(AF_INET, argv[1], port, &sock); //argv[1] : 서버의 IP
        TCP_Connect(AF_INET, argv[1], port + 1, &sock1); //void TCP_Connect(int af, char *servip, unsigned int port, int *sock)
        if (sock == -1 && sock1 == -1)
        {
            printf("FTP 연결 실패 !!\n");
            exit(1);
...
void TCP_Connect(int af, char *servip, unsigned int port, int *sock)
{
    ...
    if (connect(*sock, (struct sockaddr *)&servaddr, sizeof(servaddr)) < 0)  //int connect(int sockfd, const struct sockaddr *addr, socklen_t addrlen);
